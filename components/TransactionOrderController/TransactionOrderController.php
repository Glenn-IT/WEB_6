...
class TransactionOrderController {
    ...
    public function __construct($db) {
        $this->db = $db;
        $this->view = "TransactionOrderController";
    }
    ...
    public function index() {
        ...
        $result = $this->db->Select("
            ...
            LEFT JOIN main_order m ON
                m.main_order_id = c.main_order_id
            ORDER BY
                m.created_at desc
        ...
        foreach ($result as $key => $value) {
            $data["list"][$value["main_order_id"]][]  = $value;
        }
    ...
    public function updateStatus(){
        ...
        $this->db->Update("update orders SET val_status = ? WHERE main_order_id = ? ", array( $status,$id) );
        $this->db->Update("update main_order SET val_stattus = ? WHERE main_order_id = ? ", array( $status,$id) );

        $result = $this->db->Select("select * from orders where main_order_id = ?", array($id) )[0];
