...
class InventoryReportController {
    ...
    public function index() {
        ...
        $data["list"] = $this->db->Select(" 
            select 
            it.*,
            (select sum(c.quantity) from  cart c  INNER JOIN orders o ON o.order_no = c.order_no where o.val_status = 'COMPLETED'  and c.item_id = it.item_id ) total_sold,
            (select sum(c.quantity) from  stock_in c    where c.val_status = 'APPROVED' and c.deleted = 0 and c.item_id = it.item_id ) total_in,
            b.name  brand_name
            from items it 
            inner join brand b ON  b.id = it.brand_id
            WHERE it.status = 1 and it.deleted = 0 and it.item_type = 0", array() );
