const module = `${URL_BASED}component/brand-type/`;
const component = `component/brand-type/`;
...
// Handle form submission for brand add/edit
$(document).on("submit", ".modalOpenCustom form", function (e) {
  ...
  if (nameValue === "" || nameValue.length === 0) {
    ...
    main.alertMessage(
      "warning",
      "Validation Error",
      "Brand name cannot be empty or contain only spaces!"
    );
  ...
  if (!/\S/.test(nameValue)) {
    main.alertMessage(
      "warning",
      "Validation Error",
      "Brand name must contain at least one non-space character!"
    );
    $(this).find('input[name="name"]').focus();
    return false;
  }
